...
for arg in "$@"; do
    case "$arg" in
        --trt) TENSORRT=ON ;;
        *) echo "usage: $0 [auto|cpu|cuda] [--trt]" >&2; exit 2 ;;
    esac
done
...
if [[ "$BACKEND" != cpu && "$BACKEND" != cuda ]]; then
    echo "usage: $0 [auto|cpu|cuda] [--trt]" >&2
    exit 2
fi
...
cmake -S "$ROOT_DIR" -B "$BUILD_DIR" \
    ...
    -DDA3CPP_CUDA="$CUDA" \
    -DDA3CPP_TENSORRT="$TENSORRT"
cmake --build "$BUILD_DIR" --parallel "$JOBS"
echo "Built da3cpplib ($BACKEND, TensorRT=$TENSORRT) in $BUILD_DIR"
